...
def do_something(sitelist,match_id,input_cnfg):
    ...
    download_wind_resource_for_single_site(sitelist,match_id,input_cnfg["output_dir"],resource_year=input_cnfg["resource_year"],lat_string=input_cnfg["lat_string"],lon_string=input_cnfg["lon_string"])
...
def main(sitelist,input_info,verbose = False):
    ...
    tofu_log.debug.info(f"rank {rank}: ellapsed time: {datetime.now() - start_time}")
